...
- The MFA secret is encrypted before storage for newly created MFA users, but production systems still need careful key management. Possible solution: store the encryption key in a secret manager, rotate it safely, restrict access to it, and only expose the QR code during enrollment.
...
1. Tighten secrets and storage
   ...
   - add key rotation guidance for encrypted MFA secrets
